...
```
if pastOptimisticVotingSupply == 0: Canceled
else vetoThreshold = max(1, vetoThresholdRatio * pastOptimisticVotingSupply / 1e18)
```

Where `pastOptimisticVotingSupply = token.getPastOptimisticVotingSupply(snapshot)` is the supply that had a nonzero optimistic delegate at the snapshot, and `vetoThresholdRatio` is a D18 fraction (e.g. `0.1e18` = 10%).
