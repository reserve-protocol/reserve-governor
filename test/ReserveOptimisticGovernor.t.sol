...
//         vm.prank(alice);
//         vm.expectRevert(abi.encodeWithSelector(IReserveOptimisticGovernor.InvalidCall.selector, eoaTarget,
// callData)); governor.propose(targets, values, calldatas, "EOA call should fail");
//     }
...
//         vm.prank(optimisticProposer);
//         vm.expectRevert(abi.encodeWithSelector(IReserveOptimisticGovernor.InvalidCall.selector, eoaTarget,
// callData)); governor.proposeOptimistic(targets, values, calldatas, "EOA target");
//     }
...
//         vm.prank(optimisticProposer);
//         vm.expectRevert(abi.encodeWithSelector(IReserveOptimisticGovernor.InvalidCall.selector, eoaTarget,
// bytes(""))); governor.proposeOptimistic(targets, values, calldatas, "EOA ETH transfer");
//     }
...
//                 bytes32(
//                     uint256(
//                         (1 << uint8(IGovernor.ProposalState.Succeeded)) | (1 <<
...
//         uint256 expectedVoteEnd = block.timestamp + VOTING_PERIOD;
...
// uint256(IReserveOptimisticGovernor.ProposalType.Standard)); assertEq(governor.vetoThreshold(proposalId), 0);
//         assertEq(uint256(governor.state(proposalId)), uint256(IGovernor.ProposalState.Active));
